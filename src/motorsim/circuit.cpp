...
void CircuitSimulator::initialize(const std::vector<ScenarioFrame>& frames) {
    ...
    const auto makeTraceKey = [&](const std::string& circuitId,
                                  const std::string& coilId) {
        std::string key = circuitId + ":" + coilId;
        if (traceMetadata_.find(key) == traceMetadata_.end()) {
            return key;
        }
        std::size_t suffix = 1;
        std::string candidate;
        do {
            candidate = key + "#" + std::to_string(suffix++);
        } while (traceMetadata_.find(candidate) != traceMetadata_.end());
        return candidate;
    };
    ...
    for (const auto& specCircuit : baseSpec.circuits) {
        ...
        for (const auto& link : specCircuit.coilLinks) {
            ...
            const std::size_t linkIndex = data.coilLinks.size();
            const auto& regionSpec = baseSpec.currentRegions[link.regionIndex];
            std::string coilId = link.id.empty() ? ("coil_" + std::to_string(linkIndex)) : link.id;
            std::string regionId = regionSpec.id;
            CoilLinkData entry{};
            entry.inductorIndex = link.inductorIndex;
            entry.regionIndex = link.regionIndex;
            entry.turns = link.turns;
            entry.baseOrientation = safe_orientation(regionSpec.orientation);
            entry.currentOrientation = entry.baseOrientation;
            entry.coilId = coilId;
            entry.regionId = regionId;
            entry.traceKey = makeTraceKey(specCircuit.id, coilId);
            ...
            data.inductors[entry.inductorIndex].coilLinkIndices.push_back(linkIndex);
            TraceMetadata meta{};
            meta.circuitId = specCircuit.id;
            meta.coilId = coilId;
            meta.regionId = regionId;
            meta.turns = entry.turns;
            traceMetadata_[entry.traceKey] = meta;
            history_[entry.traceKey] = {};
...
void CircuitSimulator::update_for_frame(ScenarioFrame& frame, const MechanicalSimulator* mechanical) {
    ...
    for (auto& circuit : circuits_) {
        for (auto& link : circuit.coilLinks) {
            ...
            link.currentOrientation = orientation;
        }
    }
}
...
void CircuitSimulator::apply_currents(ScenarioFrame& frame) {
    ...
    for (std::size_t circuitIndex = 0; circuitIndex < circuits_.size(); ++circuitIndex) {
        ...
        for (const auto& link : circuit.coilLinks) {
            ...
            const double orientation = safe_orientation(link.currentOrientation);
...
void CircuitSimulator::record_solved_frame(const ScenarioFrame& frame, const Grid2D& grid) {
    ...
    for (auto& circuit : circuits_) {
        for (std::size_t idx = 0; idx < circuit.inductors.size(); ++idx) {
            ...
            const double current = state.current;
            for (std::size_t linkIndex : state.coilLinkIndices) {
                if (linkIndex >= circuit.coilLinks.size()) {
                    continue;
                }
                const auto& link = circuit.coilLinks[linkIndex];
                auto historyIt = history_.find(link.traceKey);
                if (historyIt == history_.end()) {
                    continue;
                }
                const double orientation = safe_orientation(link.currentOrientation);
                TraceSample sample{};
                sample.time = frame.time;
                sample.current = current;
                sample.effectiveCurrent = orientation * current;
                sample.ampereTurns = sample.effectiveCurrent * link.turns;
                sample.orientation = orientation;
                sample.backEmf = state.backEmf;
                historyIt->second.push_back(sample);
            }
        }
    }
}
